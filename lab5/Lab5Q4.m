...
% Get the power spectrum again to show how the filter affected the image
filtered_image = double(filtered_image);
filtered_image = filtered_image - mean(mean(filtered_image));
% 2D Bartlett window
x = bartlett(32);
for i = 1:32
    zx(i,:) = x';
...
for (i = 1:16:320)
    for (j = 1:16:288)
        Isub = filtered_image(i:i+31,j:j+31).*bartlett2D;
        P = P + fftshift(fft2(Isub,256,256));
        n = n + 1;
    end
end
